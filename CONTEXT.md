...
A Scan is **privacy-preserving by construction**: it retains only coarse, derived attributes — when the scan happened, its outcome, a coarse **geographic origin** (`country` plus its first-level **`subdivision`** — a state / province / 縣市, **never city**) and a coarse **device class** — each derived at ingest from the scanner's request and then discarded at the source. Subdivision is the **finest geographic granularity kept**: it gives a useful dashboard breakdown while staying non-identifying at low scan volume, whereas a city-level origin on a lightly-scanned Link could fingerprint an individual scanner. The geo source (GeoLite2-City) physically knows the city, but the Scan model derives only country + subdivision and discards the rest. The raw scanner IP and user agent are **never stored**, which makes ADR 0006 (the owner sees aggregates, never raw scanner identity) structurally true rather than merely enforced at display time. An owner sees **total** scan counts, not unique-visitor counts, so no per-scanner identifier is retained (not even a salted hash).
